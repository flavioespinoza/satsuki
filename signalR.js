...
function negotiateProxies (baseUrl, hubNames, onSuccess, onError, _client) {
  ...
  let cleanedHubs = toCleanHubNames(hubNames)
  ...
  let negotiateData = ''
  let negotiateUrl = baseUrl + '/negotiate?' + querystring.stringify({
    connectionData: JSON.stringify(cleanedHubs),
    clientProtocol: 1.5
  })
  let negotiateUrlOptions = url.parse(negotiateUrl, true)

  let negotiateFunction = function (res) {
    ...
    res.on('end', function (endRes) {
      try {
        if (res.statusCode == 200) {
          let negotiateObj = JSON.parse(negotiateData)
  ...
  let negotiateErrorFunction = function (e) {
    _client.connection.state = states.connection.bindingError
    if (_client.serviceHandlers.bindingError) {
      _client.serviceHandlers.bindingError(e)
    } else {
  ...
  if (_client.headers) {
    for (let propName in _client.headers) {
  ...
  if (negotiateUrlOptions.protocol === 'http:') {
    let negotiateResult = http.get(negotiateUrlOptions, negotiateFunction).on('error', negotiateErrorFunction)
  } else if (negotiateUrlOptions.protocol === 'wss:') {
    negotiateUrlOptions.protocol = 'https:'
    let negotiateResult = https.get(negotiateUrlOptions, negotiateFunction).on('error', negotiateErrorFunction)
  } else {
...
function getConnectQueryString (_client) {
  ...
  if (_client.queryString) {
    for (let propName in _client.queryString) {
      qs[propName] = _client.queryString[propName]
    }
  }

  let connectQueryString = _client.url + '/connect?' + querystring.stringify(qs)
...
function getAbortQueryString (_client) {
  ...
  let abortQueryString = _client.url + '/abort?' + querystring.stringify(qs)
...
function getStartQueryString (_client) {
  ...
  let startQueryString = _client.url + '/start?' + querystring.stringify(qs)
...
function clientInterface (baseUrl, hubs, reconnectTimeout, doNotStart) {
  ...
  client.on = function (hubName, methodName, callback) {
    let handler = _client.handlers[hubName.toLowerCase()]
    if (!handler) {
      handler = _client.handlers[hubName.toLowerCase()] = {}
    }
    let method = handler[methodName.toLowerCase()] = callback
  }
  client.end = function (__reason) {
    console.log('end() --> reason: ', __reason)
    if ((_client.connection.state == states.connection.connecting
      ...
      let connection = _client.websocket.connection
      _client.websocket.connection = undefined
      connection.close()
    }
  }
  client.invoke = function (hubName, methodName) {
    let hub = client.hub(hubName)
    if (!hub)
      return
    let args = getArgValues(arguments)
    return hub.invoke.apply(hub, args)
  }

  let callTimeout = 30000
  let callCallbacks = {}
  ...
  client.call = function (hubName, methodName) {
    let nohub = typeof client.invoke.apply(client, arguments) === 'undefined'
    return {
        ...
        let messageId = client.lastMessageId
  ...
  function handleCallResult (messageId, err, result) {
    let cb = callCallbacks[messageId]
    if (cb) cb(err, result)
  }
  ...
  _client.invoke = function (_hub, methodName, args) {
    ...
    let payload = buildPayload(_hub.data.name, methodName, args, ++_client.websocket.messageid)
  ...
  function scheduleReconnection (isInitalRetry) {
    ...
    if (_client.connection.state == states.connection.retryingConnection) {
      ...
      if (_client.serviceHandlers.reconnecting) {
        let retry = {inital: isInitalRetry, count: _client.websocket.reconnectCount}
        cancelRetry = _client.serviceHandlers.reconnecting.apply(client, [retry])
      }
      if (!cancelRetry) {
        setTimeout(function () {
          let connectQueryString = getConnectQueryString(_client)
  ...
  function abort () {
    ...
    if (_client.connection.state === states.connection.disconnected || _client.connection.state === states.connection.disconnecting) {
      ...
      let abortQueryString = getAbortQueryString(_client)

      let abortUrlOptions = url.parse(abortQueryString, true)
      ...
      let req = requestObject.request(abortUrlOptions,
        function (res) {
  ...
  function startCommunication (onSuccess, onError) {
    ...
    let startUrl = getStartQueryString(_client)

    let startUrlOptions = url.parse(startUrl, true)

    let startData = ''
    let startFunction = function (res) {
      res.on('data', function (chunk) {
        startData += chunk
      })
      res.on('end', function (endRes) {
        try {
          if (res.statusCode == 200) {
            let startObj = JSON.parse(startData)
            onSuccess(startObj)
          } else if (res.statusCode == 401 || res.statusCode == 302) {
            if (_client.serviceHandlers.onUnauthorized) {
              _client.serviceHandlers.onUnauthorized(res)
    ...
    let startErrorFunction = function (e) {
      _client.connection.state = states.connection.bindingError
      if (_client.serviceHandlers.bindingError) {
        _client.serviceHandlers.bindingError(e)
      } else {
    ...
    if (startUrlOptions.protocol === 'http:') {
      let startResult = http.get(startUrlOptions, startFunction).on('error', startErrorFunction)
    } else if (startUrlOptions.protocol === 'wss:') {
      startUrlOptions.protocol = 'https:'
      let startResult = https.get(startUrlOptions, startFunction).on('error', startErrorFunction)
    } else {
        ...
        if (message.type === 'utf8' && message.utf8Data != '{}') {
          ...
          if (parsed.hasOwnProperty('M')) {
            for (let i = 0; i < parsed.M.length; i++) {
              let mesg = parsed.M[i]
              let hubName = mesg.H.toLowerCase()
              let handler = _client.handlers[hubName]
              if (handler) {
                let methodName = mesg.M.toLowerCase()
  ...
  _client.getBinding = function () {
    getBindings(baseUrl, hubs, function (bindings) {
      ...
      //hubs:
      for (let i = 0; i < bindings.hubs.length; i++) {
        let hubData = bindings.hubs[i]

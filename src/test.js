...
let __chart_interval = 'hour'
let __last = null
let __bid = null
let __ask = null
let __limit = 10080

let __auto_trade_buy = true
let __auto_trade_sell = true

let auto_sell_percentage = 2.00
let auto_buy_percentage = 5.00
...
const MongoDB_URL = 'mongodb://heroku_zqx25kj9:md9hm8e9r13id0dekt6tljt9ua@ds127993.mlab.com:27993/heroku_zqx25kj9'
...
const d3_time = require('d3-time-format')
...
const sms = new twilio('ACf7059162ffbdbdde9fe342ebb43b06b1', '5ad7ac8319175a8c63d7e37c5c9b2f9d')
function ___send__SMS (__message) {
  if (__hold_sms == 0) {
    __hold_sms++
    sms.messages.create({
      // to: '8019494666', // Miah
    ...
    // log.yellow('Text Message Sent: ' + __message)
  }
}
...
(async function () {
  ...
  log.cyan('orderHistory', JSON.stringify(orderHistory))
...
async function ___all_markets (__socket) {
  ...
  function swap (chars, counter_i, i) {
    let temp = chars[counter_i]
    chars[counter_i] = chars[i]
    chars[i] = temp
  }
  ...
  try {
    ...
    let fetchAllMarkets = await XCH.fetchMarkets()

    __socket.emit('all_markets', {
      all_markets: fetchAllMarkets,
      market: __market_name,
      symbol: __market_symbol,
      all_buy_orders: [],
      all_sell_orders: [],
...
async function ___REPLACE_limit_buy_order (__buy_order, __socket) {
  ...
  try {
    ...
    let place_buy_order = await XCH.create_limit_buy_order(__buy_order.symbol, __buy_order.amount, __toFixed(__buy_order.price, null, 2, 5))
...
async function ___REPLACE_limit_sell_order (__sell_order, __socket) {
  ...
  try {
    ...
    let place_sell_order = await XCH.create_limit_sell_order(__sell_order.symbol, __sell_order.amount, __toFixed(__sell_order.price, null, 2, 5))
  ...
  if (__replace_order.side === 'sell') {
    ...
    try {
      ...
      let new_order = {
        ...
        price: __toFixed(__replace_order.NewPrice, null, 2, 5),
        type: 'limit'
      }
    ...
    try {
      ...
      let new_order = {
        market: __replace_order.symbol,
        symbol: __replace_order.symbol,
        amount: __toFixed(new_amount, null, 2, 5),
        price: __toFixed(__replace_order.NewPrice, null, 2, 5),
        type: 'limit'
      }
  ...
  try {
    ...
    if (__order.side === 'buy') {
      if (__order.side === 'buy' && auto_sell_toggle) {
        ...
        let final_sell_price = __toFixed(sell_price, null, 2, 5)
      ...
      if (__order.side === 'sell' && auto_buy_toggle) {
        ...
        let final_buy_price = __toFixed(buy_price, null, 2, 5)
...
function __exp (__obj) {
  return  JSON.stringify(__obj, null, 2)
}

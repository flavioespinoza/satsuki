...
function helper({ x1, y1, x2, y2 }) {
  ...
  const zero = _.find(rulers, {percent:0})

  _.each(rulers, function (obj, i) {
    let decimal = ((obj.y / zero.y) - 1)
    rulers[i].percentAboveZero = decimal * 100
  })

  // console.log('rulers', JSON.stringify(rulers, null, 2))

  return rulers;
}


class EachRuler extends Component {
  constructor(props) {
    super(props);
  ...
  render() {
    ...
    return <g>
      {lines.map((line, j) => {
        const text = `${yDisplayFormat(line.y)} ~ ${line.percentAboveZero.toFixed(2)}%`;
